...
/**
 * @desc: 网络请求工具类---使用的是全局配置的变量
 * @date: 2018/2/9
 * @author: YangYuKun 04118
 */

public class GlobalRxHttp {
    private static volatile GlobalRxHttp instance = null;
     ...
     * @return
     */
    public GlobalRxHttp setLog(boolean isShowLog, HttpLoggingInterceptor.Level level) {
        if (isShowLog) {
    ...
    public GlobalRxHttp setCache() {
        ...
        Cache cache = new Cache(new File(Environment.getExternalStorageDirectory().getPath() + "/rxHttpCacheData")
                , 1024 * 1024 * 100);
        getGlobalOkHttpBuilder().addInterceptor(httpCacheInterceptor)
    ...
    public GlobalRxHttp setWriteTimeout(long second) {
        ...
        getGlobalOkHttpBuilder().writeTimeout(second, TimeUnit.SECONDS);
        return this;
    }
    ...
    public GlobalRxHttp setConnectTimeout(long second) {
        ...
        getGlobalOkHttpBuilder().connectTimeout(second, TimeUnit.SECONDS);
        return this;
    }
